...
class Graph:
    ...
    async def create_cluster(self, nodes):
        ...
        cluster.text = generate_summary(cluster_text)
    ...
    async def search(self, query, embeddings):
...
def generate_summary(text):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Summarize the following text in a single paragraph:\n\n{text}"}
        ]
    )
...
async def main():
    ...
    build_parser.add_argument("--chunk_size", type=int, default=200, help="Size of text chunks")
    ...
    if args.command == "build":
        ...
        results = await graph.search(args.query, embeddings)
        
        processed_results = []
        included_summaries = set()
        ...
        for node, score, depth, parent_cluster in results:
            ...
            if parent_cluster and parent_cluster.text not in included_summaries:
                result['cluster_summary'] = parent_cluster.text
                included_summaries.add(parent_cluster.text)
            elif node.is_cluster and node.text not in included_summaries:
                result['cluster_summary'] = node.text
                included_summaries.add(node.text)
        ...
        if args.output_file:
            ...
            for result in processed_results:
                print(f"{'Cluster: ' if result['is_cluster'] else 'Document: '}{result['name']}")
                print(f"Text: {result['text'][:200]}...")
                if 'cluster_summary' in result:
                    print(f"Cluster Summary: {result['cluster_summary'][:200]}...")
